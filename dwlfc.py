...
def cam_config():
    ...
    if mode in {"pta", "ptm", "tla", "tlm", "ado"}:
        ...
        still = cam.create_still_configuration(raw={}, lores= {"size": (320, 240), "format": "YUV420"})
        cam.configure(still)
    elif mode in {"vda", "vdm"}:
        video = cam.create_video_configuration(main= {"size": (1456, 1088), "format": "RGB888"}, lores= {"size": (320, 240), "format": "YUV420"})
...
# from: https://github.com/raspberrypi/picamera2/blob/main/examples/zoom.py
    if x == 0:
        # resets the zoom!
        cam.controls.ScalerCrop = (0, 0, 1456, 1088)
    else:
            ...
            if disable_preview == False:
                ...
                if mode in {"pta", "ptm"}:
                    ...
                    if recording == False:
                        ...
                        # TODO
                        # gotta add a "audio_recorded" track if we have "intrisic audio == true"
                        # as well account for audio_recorded time when we are in +mono/stereo/etc. modes
